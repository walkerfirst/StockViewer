...
class DataEngineSina(object):
    ...
    def __request(self, symbol):
        ...
        headers = ("User-Agent"," Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko)"
                                " Chrome/66.0.3359.139 Safari/537.36")  # 这里模拟浏览器
    ...
    def getStockData(self):
        ...
        for textPiece in dataArray:
            ...
            list = [code,name,price,pre_close,changeNum,time]
            new_list.append(list)
        df = pd.DataFrame(new_list,columns=['code','name','price','pre_close','change_rate','time'])
        return df
...
class DataEngine(object):
    ...
    def qryAccount(self,accountName = 'all'):
        ...
        _columns = ['code','cash', 'cost','profit']
        # print(columns)
        # 得到持仓dataframe
        if accountName == 'all':
    ...
    def getAccDict(self):
        ...
        for name in self.account:
            self.accDict[name] = accdb[accdb['code'] == name].sum()
        accdb_all = accdb.sum()
        # 重置series的 name值
        accdb_all['code'] = 'all'
    ...
    def processData(self,left,right):
        """处理两个df，得到持仓股票需要数据格式"""
        self.df = pd.merge(left, right, on='code')
        ...
        self.df = self.df[['code', 'name', 'qty','position', 'cost', 'price', 'value', 'profit_day', 'change_rate', 'profit',
                           'profit_rate', 'time']].sort_values(by='change_rate', ascending=False)
        return self.df
