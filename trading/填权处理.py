...
from util.access_db import read_access_db,process_access_db
from trading.event_trigger import accountEvent
...
class stcokprocess(QWidget):
    ...
    def addNum(self):
        ...
        df = read_access_db(read_sql,column_list=['id','code','name','cost','qty'])
        ...
        for index in df.index:
            ...
            process_access_db(update_sql)
            print('update is done')
        QMessageBox.about(self, "对话框", "填权处理已完成！")
        self.close()

        # 发送账号变动event
        accountEvent(self.mainEngine)
